...
TensorModel::TensorModel(const std::string& model_filename, const std::vector<uint8_t>& config_options) {
...
void TensorModel::save(const std::string &ckpt) {
...
void TensorModel::restore(const std::string& ckpt) {
...
TF_Buffer * TensorModel::read(const std::string& filename) {
...
std::vector<std::string> TensorModel::get_operations() const {
...
void TensorModel::run(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
void TensorModel::run(Tensor &input, Tensor &output) {
    this->run(&input, &output);
}

void TensorModel::run(const std::vector<Tensor*> &inputs, Tensor &output) {
    this->run(inputs, &output);
}

void TensorModel::run(Tensor &input, const std::vector<Tensor*> &outputs) {
...
bool TensorModel::status_check(bool throw_exc) const {
...
void TensorModel::error_check(bool condition, const std::string &error) const {
    if (!condition) {
        throw std::runtime_error(error);
    }
}

TensorModel* TensorModel::TFInstance = nullptr;

TensorModel* TensorModel::Instance(const std::string& model_filename, const std::vector<uint8_t>& config_options)
{
    TFInstance = new TensorModel(model_filename, config_options);
    return TFInstance;
}
